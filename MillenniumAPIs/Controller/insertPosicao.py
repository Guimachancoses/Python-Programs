...
def insert_posicao_data(connection, data, csv_file_path=r'\\AGENDA\arq\temp_data.csv'):
    cursor = connection.cursor()
    total_inserted = 0  # Contador para total de registros inseridos
    temp_table = "#TempPosicoes"  # Tabela temporária no SQL Server

    # Conta o total de registros no array
    tdRegistros = contar_registros(data)
    ...
    try:
        ...
        CREATE TABLE {temp_table} (
            GPSPositionTimestamp NVARCHAR(255),
            InsertedTimestamp NVARCHAR(255),
        ...
        with open(csv_file_path, mode='w', newline='', encoding='utf-8') as file:
            ...
            writer.writerow([
                "GPSPositionTimestamp", "InsertedTimestamp", "Address", "Company", "VehicleID", "Plate",
                "DeviceID", "DeviceType", "Driver", "GPSSpeed", "CANSpeed", "Odometer", "Ignition",
                "Latitude", "Longitude", "Heading", "RPM", "URL"
            ])

            for row in data:
                writer.writerow([
                    sanitize_value(row["GPS positiontimestamp"]), sanitize_value(row["insertedtimestap"]),
                    sanitize_value(row["address"]), sanitize_value(row["Company"]), sanitize_value(row["vehicleid"]),
                    sanitize_value(row["plate"]), sanitize_value(row["deviceid"]), sanitize_value(row["Device type"]),
                    sanitize_value(row["driver"]), sanitize_value(row["gpsspeed"]), sanitize_value(row["canspeed"]),
                    sanitize_value(row["odometer"]), sanitize_value(row["ignition"]), sanitize_value(row["latitude"]),
                    sanitize_value(row["longitude"]), sanitize_value(row["heading"]), sanitize_value(row["rpm"]),
        ...
        merge_query = f"""
        MERGE INTO POSICOES AS target
        USING {temp_table} AS source
        ON target.GPSPositionTimestamp = source.GPSPositionTimestamp AND target.VehicleID = source.VehicleID
        WHEN NOT MATCHED BY TARGET THEN
            INSERT (GPSPositionTimestamp, InsertedTimestamp, Address, Company, VehicleID, Plate, DeviceID,
                    DeviceType, Driver, GPSSpeed, CANSpeed, Odometer, Ignition, Latitude, Longitude, Heading, RPM, URL)
            VALUES (source.GPSPositionTimestamp, source.InsertedTimestamp, source.Address, source.Company, source.VehicleID, source.Plate, source.DeviceID, source.DeviceType,
                    source.Driver, source.GPSSpeed, source.CANSpeed, source.Odometer, source.Ignition, source.Latitude, source.Longitude, source.Heading, source.RPM, source.URL);
        """
...
def adjust_dates(last_event_timestamp):
    ...
    if (hour == 0 and minute >= 0) or (hour < 2 and minute <= 59):
        dateToaj = dateFromaj.replace(hour=2, minute=59, second=0)

    elif (hour == 3 and minute >= 0) or (hour < 5 and minute <= 59):
        dateToaj = dateFromaj.replace(hour=5, minute=59, second=0)

    elif (hour == 6 and minute >= 0) or (hour < 8 and minute <= 59):
        dateToaj = dateFromaj.replace(hour=8, minute=59, second=0)

    elif (hour == 9 and minute >= 0) or (hour < 11 and minute <= 59):
        dateToaj = dateFromaj.replace(hour=11, minute=59, second=0)

    elif (hour == 12 and minute >= 0) or (hour < 14 and minute <= 59):
        dateToaj = dateFromaj.replace(hour=14, minute=59, second=0)

    elif (hour == 15 and minute >= 0) or (hour < 17 and minute <= 59):
        dateToaj = dateFromaj.replace(hour=17, minute=59, second=0)

    elif (hour == 18 and minute >= 0) or (hour < 20 and minute <= 59):
        dateToaj = dateFromaj.replace(hour=20, minute=59, second=0)

    elif (hour == 21 and minute >= 0) or (hour < 23 and minute <= 59):
        dateToaj = dateFromaj.replace(hour=23, minute=59, second=0)

    elif (hour == 23 and minute >= 0) or (hour <= 23 and minute <= 59):
        dateToaj = (last_event_timestamp + timedelta(days=1)
                    ).replace(hour=2, minute=59, second=0)
...
def MainPosicao():
    query = "SELECT MAX(GPSPositionTimestamp) AS LastEventTimestamp FROM POSICOES;"
    json_data = [1]
    ...
    while json_data and (dateToc <= horaAtual2):
        ...
        if connection:
            ...
            url = f"https://api3-ng.tracknow.com/reports/positionreport.php?userlanguageid=3&timezone=America/Sao_Paulo&datefrom={datefrom}&dateto={
                dateTo}&vehiclegroupid=&vehiclebrandid=&cargotypeid=&deviceid=&devicetypeid=19&devicebrandid=&companyid%5B%5D=361&driverid=&countryid=&plate=&canspeedoperator1=geq&canspeed1=&canspeedoperator2=leq&canspeed2=&gpsspeedoperator1=geq&gpsspeed1=&gpsspeedoperator2=leq&gpsspeed2=&rpmoperator1=geq&rpm1=&rpmoperator2=leq&rpm2=&uploadprotocol=&showgpsspeed=set&showcanspeed=set&showignition=set&showodometer=set&showlatitude=set&showlongitude=set&showheading=set&showrpm=set&showaddress=set&showpositiontime=set&showdatecreated=set&gbcompany=set&gbdeviceid=set&gbvehicleid=set&gbvehicle=set&gbdriver=set"

        # Exemplo de como chamar fetchEventReport para obter os dados
            json_data = fetchEventReport(url)

            if json_data and "rows" in json_data:
                totalInsert = insert_posicao_data(
